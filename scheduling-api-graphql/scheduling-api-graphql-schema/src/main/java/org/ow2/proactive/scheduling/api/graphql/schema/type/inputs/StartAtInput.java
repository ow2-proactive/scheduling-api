...
public class StartAtInput extends ComparableLongInput {
    ...
    public final static TypeSingleton<GraphQLInputType> TYPE = new TypeSingleton<GraphQLInputType>() {
        ...
        public GraphQLInputType buildType(DataFetcher... dataFetchers) {
            return newInputObject().name(Types.START_AT.getName())
                                   .description("Start At input.")
                                   .field(newInputObjectField().name(BEFORE.getName())
                                                               .description("Jobs having their start at lower than this value.")
                                                               .type(GraphQLLong)
                                                               .build())
                                   .field(newInputObjectField().name(AFTER.getName())
                                                               .description("Jobs having their start at greater than this value.")
                                                               .type(GraphQLLong)
                                                               .build())
                                   .field(newInputObjectField().name(NULL_STATUS.getName())
                                                               .description("Is the start at value present")
                                                               .type(NullStatusEnum)
                                                               .build())
                                   .build();
        }
    ...
    public StartAtInput(Map<String, Object> input) {
        super(input);
    }
}
